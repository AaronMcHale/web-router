...
For example, in `docker-compose.traefik-api-dashboard.yml` we extend `traefik` service and add additional configuration environment variables to enable the API and Dashboard. When Docker merges the compose files together, all of the configuration will be passed into the Traefik container for use at runtime.
...
When running `docker compose up`, assuming this service is enabled, the `docker-compose.yml` file will be merged with the other Compose files, and the following would happen:
...
2. It notices that we're providing some environment variables, and because the environment variables are a list, Docker will merge together all `environment` lists into a single list on the Traefik container.
